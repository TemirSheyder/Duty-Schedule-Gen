...
function initDefaultMonths() {
  ...
  const elFrom = document.getElementById("tMonthFrom");
  const elTo   = document.getElementById("tMonthTo");
  const elPrev = document.getElementById("tPreviewMonth");

  if (elFrom && !elFrom.value) elFrom.value = formatMonth(from);
  if (elTo   && !elTo.value)   elTo.value   = formatMonth(to);
  if (elPrev && !elPrev.value) elPrev.value = formatMonth(from);
}
