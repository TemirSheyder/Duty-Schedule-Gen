...
export function buildSvgForMonth(P, year, month, scale, processedImages) {
  ...
  const schedule = buildFullSchedule(
    ...
    new Date(P.work.lastDate + "T00:00:00"),
  ...
  // ВАЖНО: никаких clamp'ов для barH/artH/artTop/artRight/wdY.
  // Шаблоны могут использовать крайние значения осознанно (см. tmplBlack).
  const barH = P.bar.h;
  const artH = P.art.h;
...
function setupWheelZoom() {
  ...
  if (!wrap._pinchBound) {
    ...
    wrap.addEventListener("touchstart", (e) => {
      ...
      startScale = parseFloat(document.getElementById("pScale").value);
    }, { passive: true });
